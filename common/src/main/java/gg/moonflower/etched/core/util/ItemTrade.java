...
public class ItemTrade implements VillagerTrades.ItemListing {
    ...
    public ItemTrade(Supplier<? extends ItemLike> Item, int emeralds, int itemCount, int maxUses, int xpGain) {
        this(Item, emeralds, itemCount, maxUses, xpGain, 0.05F, false);
    }

    public ItemTrade(Supplier<? extends ItemLike> Item, int emeralds, int itemCount, int maxUses, int xpGain, boolean sellToVillager) {
        this(Item, emeralds, itemCount, maxUses, xpGain, 0.05F, sellToVillager);
    }

    public ItemTrade(Supplier<? extends ItemLike> Item, int emeralds, int itemCount, int maxUses, int xpGain, float priceMultiplier, boolean sellToVillager) {
        this.item = Item;
